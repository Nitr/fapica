class ApiController < ApplicationController
  ...
  respond_to :json
	...
	def get_device
		render :json => Device.where(:terminal_id => current_user.company.terminals.map {|t| t.id}).joins(:device_type, :terminal).select('devices.*, device_types.name as device_type_name, terminals.hashkey')
	end
	def delete_device
